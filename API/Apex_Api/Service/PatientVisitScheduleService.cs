            ...
            {         
                                                                           ...
                                                                           x.colorType == "D" && x.IsCombined == true)))
                {
                    ...
                    if (i == 0)
                    {
                        if (lasttime != null)
                        {
                            // Get Distance from last patient to next patient
                            var patientDetails = Common.Instances.VisitScheduleInst.GetPatientLatLon(item.PatientId);
                            var distanceMatrix = new GoogleMapsService.DistanceData();
                            if (lastScheduleByDate != null)
                            {
                                distanceMatrix = new GoogleMapsService().GetDistanceMatrix(lasttime.PatientLat, lasttime.PatientLong,
                                patientDetails.Lat, patientDetails.Long);
                            }
                            ...
                            visitScheduleResponse.Start = visitSchedule.VisitDate.Date != lasttime.End.Date
                                ...
                                : lastScheduleByDate.End.AddMinutes(distanceMatrix.DurationMins);
